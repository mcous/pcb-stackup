...
export function valuesToBoardUpdate(values: Values): BoardUpdate {
  let outlineGapFill =
  ...
  return {
    ...
    gerberOptions: parseFormLayerOptions(values.gerberOptions),
    drillOptions: parseFormLayerOptions(values.drillOptions),
...
function parseFormLayerOptions(
  ...
  if (Array.isArray(coordinateFormat)) {
    ...
    if (Number.isFinite(parsed[0]) && Number.isFinite(parsed[1])) {
      ...
      log.warn(`Invalid coordinateFormat: ${JSON.stringify(coordinateFormat)}`)
    }
  }
